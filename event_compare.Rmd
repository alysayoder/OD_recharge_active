...
```{r post discharge calc, include = FALSE}
...
* Hover over bars to see exact start date and recharge volume of that event. 
```{r}
width = as.vector.factor(rech$Length*100)
t <- plot_ly(rech, x = rech$Start[1:7], y = rech$`Recharge Volume (m3)`[1:7], type ='bar', 
             name = "pre levee removal", marker = list(color = "#43a2ca"), width = width[1:7]) %>%
  add_trace(x = rech$Start[8:26], y=rech$`Recharge Volume (m3)`[8:26], 
            name = "post levee removal", marker = list(color= "#a8ddb5") , width = width[8:26]) %>%
  layout(
    title = "Recharge Volume of Each Event",
    xaxis = list(title = 'Time', zeroline = TRUE),
    yaxis = list(title = 'Recharge Volume (m3)')
  ) %>%
  rangeslider()
t
```

## Other Recharge Event Volume Plots
```{r plot rech vols}
library(ggplot2)
library(plyr)
library(plotly)
