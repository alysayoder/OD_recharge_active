...
begin <- lo2015
hi2015$level <- as.numeric(as.character(hi2015))
end <- hi2015
...
plot(event_rast)
title(main = "Change in Water Levels 2014WY (m)")
...
hi2015 %>% as.data.frame %>% ggplot(aes(long, lat)) + geom_point(aes(size = level), 
  color = "blue", alpha = 3/4) + ggtitle("Lowest Surface Water Level (m)") + 
